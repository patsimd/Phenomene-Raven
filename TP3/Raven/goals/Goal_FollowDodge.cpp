...
//-----------------------------------------------------------------------------
Goal_FollowDodge::
Goal_FollowDodge(Raven_Bot*          pBot,
...
//------------------------------ Activate -------------------------------------
//-----------------------------------------------------------------------------
void Goal_FollowDodge::Activate()
{
	m_iStatus = active;
	...
	case NavGraphEdge::normal:
	{
		...
		//Si un ennemi est en vu, on l'évite avant de poursuivre notre chemin
		if (m_pOwner->Health() > 25 && m_pOwner->GetTargetSys()->isTargetWithinFOV())
		{
